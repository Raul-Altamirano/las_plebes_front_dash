...
const ColorModal = ({ open, initial, onClose, onSave, saving }: ColorModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ name: form.name.trim(), hex: normalizeHex(form.hex) }, initial?.id);
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <label className="w-10 h-10 rounded-lg border border-gray-300 cursor-pointer overflow-hidden shadow-sm flex-shrink-0 relative">
                <input
                  type="color"
                ...
                />
                <div className="w-full h-full" style={{ backgroundColor: normalizeHex(form.hex) }} />
              </label>
              <input
                type="text"
                value={form.hex}
...
const ColorCard = ({ color, onEdit, onDelete, confirmDeleteId, onConfirmDelete, onCancelDelete, deleting }: ColorCardProps) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
      <div className="h-28 relative" style={{ backgroundColor: color.hex }}>
        <span
          className="absolute bottom-2 right-2 text-xs font-mono px-2 py-0.5 rounded-full bg-black/20 backdrop-blur-sm"
        ...
        {!isConfirming && (
          <div className="absolute inset-0 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-black/20">
            <button onClick={() => onEdit(color)} className="bg-white text-gray-700 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-gray-100 transition-colors shadow">
              Editar
            </button>
            <button onClick={() => onDelete(color)} className="bg-white text-red-600 rounded-lg px-3 py-1.5 text-xs font-medium hover:bg-red-50 transition-colors shadow">
              Eliminar
            </button>
          </div>
        )}
        ...
        {isConfirming ? (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={onCancelDelete} disabled={deleting} className="flex-1 text-xs py-1.5 rounded-md border border-gray-300 text-gray-600 hover:bg-gray-50 transition-colors">
                Cancelar
              </button>
              <button onClick={() => onConfirmDelete(color.id)} disabled={deleting} className="flex-1 text-xs py-1.5 rounded-md bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50">
                {deleting ? '...' : 'Eliminar'}
              </button>
            </div>
          </div>
...
const ColorsPage = () => {
  const { colors, status, createColor, updateColor, deleteColor, refresh } = useColors();
...
const handleSave = async (formData: ColorFormData, id?: string) => {
  setSaving(true);
  const enriched = { ...formData, slug: toSlug(formData.name) }; // ← agrega slug
  try {
    if (id) {
      await updateColor(id, enriched);
      showToast('Color actualizado', 'success');
    } else {
      await createColor(enriched);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Catálogo de colores disponibles para variantes de productos</p>
        </div>
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-colors shadow-sm"
        >
        ...
        {!loading && colors.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm mt-1">Crea tu primer color para usarlo en las variantes de tus productos</p>
            </div>
            <button onClick={openCreate} className="mt-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition-colors">
              Crear primer color
            </button>
          </div>
        )}
