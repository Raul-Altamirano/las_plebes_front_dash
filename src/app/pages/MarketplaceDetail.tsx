...
import { ArrowLeft, Loader2, RefreshCw, XCircle, CheckCircle2, Clock, AlertTriangle, Search } from 'lucide-react';
...
export function MarketplaceDetail() {
  ...
  const handleUnpublishAll = async () => {
    if (!confirm(`¿Estás seguro? Esto quitará todos tus productos de ${PLATFORM_SHORT_LABELS[platform]}.`)) {
      return;
    }
  ...
  const handleToggleProduct = async (productId: string, enabled: boolean) => {
    try {
      ...
      showToast(
        enabled ? 'Producto agregado a la plataforma' : 'Producto quitado de la plataforma',
        'success'
      );
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className={`inline-flex items-center px-3 py-1 rounded text-sm font-medium ${CONNECTION_STATUS_COLORS[connection.status]}`}>
