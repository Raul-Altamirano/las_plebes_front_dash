...
const INITIAL_CONNECTIONS: MarketplaceConnection[] = [
  { platform: 'FACEBOOK',  status: 'CONNECTED',    accountName: 'Las Plebes', connectedAt: new Date().toISOString() },
...
export function MarketplacesProvider({ children }: { children: ReactNode }) {
  ...
  const loadFacebookProducts = async () => {
    try {
      ...
      const statuses: ProductMarketplaceStatus[] = allProducts.map(product => {
        const published = publishedProducts.find(p => p._id === (product as any)._id || p._id === product.id);
  ...
  const connectPlatform = async (platform: MarketplacePlatform) => {
    ...
    setState(prev => ({
      ...
      connections: prev.connections.map(c =>
        c.platform === platform
          ? { ...c, status: 'CONNECTED', accountName: 'Las Plebes', connectedAt: new Date().toISOString() }
          : c
      ),
    }));
    auditLog('MARKETPLACE_CONNECTED', 'marketplace', platform, { platform, userId: currentUser?.id });
  };
  ...
  const disconnectPlatform = async (platform: MarketplacePlatform) => {
    ...
    setState(prev => ({
      ...
      connections: prev.connections.map(c =>
        c.platform === platform
          ? { ...c, status: 'DISCONNECTED', accountName: undefined, connectedAt: undefined }
          : c
      ),
    ...
    auditLog('MARKETPLACE_DISCONNECTED', 'marketplace', platform, { platform, userId: currentUser?.id });
  };
    ...
    try {
      if (platform === 'FACEBOOK') {
        ...
        auditLog('MARKETPLACE_SYNC_ALL', 'marketplace', platform, {
          platform, synced: result.synced, errors: result.errors.length, userId: currentUser?.id,
        });
      } else {
        ...
        auditLog('MARKETPLACE_SYNC_ALL', 'marketplace', platform, { platform, userId: currentUser?.id });
      }
    } finally {
  ...
  const unpublishAllProducts = async (platform: MarketplacePlatform) => {
    ...
    auditLog('MARKETPLACE_UNPUBLISH_ALL', 'marketplace', platform, { platform, userId: currentUser?.id });
  };
...
if (enabled) {
  ...
  try {
    ...
    setState(prev => ({
      ...
      productStatuses: prev.productStatuses.map(s =>
        s.productId === productId && s.platform === 'FACEBOOK'
          ? { ...s, syncStatus: 'ERROR', isPublished: false, errorMessage: err?.message || 'Error al sincronizar' }
          : s
      ),
    }));
  }
  ...
  setState(prev => ({
    ...
    auditLog('MARKETPLACE_PRODUCT_TOGGLED', 'marketplace', `${platform}-${productId}`, {
      platform, productId, enabled, userId: currentUser?.id,
    });
  };
