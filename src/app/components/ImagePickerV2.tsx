...
type PendingImage = {
  id: string; url: string; alt: string; isPrimary: boolean; _pending: true; _file: File;
};
type AnyImage = UploadedImage | PendingImage;
function isPending(img: AnyImage): img is PendingImage { return (img as PendingImage)._pending === true; }
...
export function ImagePickerV2({ images, onChange, error, disabled = false, maxImages = MAX_IMAGES, productId, categoryId, sku, uploadRef }: ImagePickerV2Props) {
  ...
  const [allImages, setAllImages] = useState<AnyImage[]>(images.map((img) => ({ ...img, _pending: false as const })));
  ...
  const uploadPending = useCallback(async (): Promise<ProductImage[] | null> => {
    ...
    const hasPrimaryChange = uploaded.some(u => { const orig = images.find(i => getImageFingerprint(i) === getImageFingerprint(u)); return orig && orig.isPrimary !== u.isPrimary; });
    ...
    const result = allImages.map((img) => {
      ...
      return { id: img.id, url: up?.publicUrl ?? img.url, alt: img.alt, isPrimary: up?.isPrimary ?? img.isPrimary, key: up?.key };
    });
    setAllImages(result);
  ...
  const handleFileSelect = (files: FileList | null) => {
    ...
    if (availableSlots <= 0) { alert(`Máximo ${maxImages} imágenes permitidas`); return; }
    const filesToAdd = fileArray.slice(0, availableSlots);
    const newPending: PendingImage[] = [];
    for (const file of filesToAdd) {
      if (!DEFAULT_UPLOAD_CONFIG.allowedTypes.includes(file.type)) { alert(`Tipo no permitido: ${file.name}\nSolo JPEG, PNG, WebP`); continue; }
      if (file.size > DEFAULT_UPLOAD_CONFIG.maxSizeBytes) { const mb = DEFAULT_UPLOAD_CONFIG.maxSizeBytes / (1024 * 1024); alert(`Archivo muy grande: ${file.name}\nMáximo ${mb}MB`); continue; }
      newPending.push({ id: `pending-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, url: URL.createObjectURL(file), alt: file.name.replace(/\.[^/.]+$/, ''), isPrimary: allImages.length === 0 && newPending.length === 0, _pending: true, _file: file });
    }
  ...
  const handleConfirmRemove = () => {
    ...
    setAllImages((prev) => {
      ...
      if (filtered.length > 0 && !filtered.some((i) => i.isPrimary)) filtered[0] = { ...filtered[0], isPrimary: true };
      const uploaded = filtered.filter((i): i is UploadedImage => !isPending(i)).map(({ _pending, ...rest }) => rest as ProductImage);
      onChange(uploaded);
      return filtered;
    });
  ...
  const handleSetPrimary = (id: string) => setAllImages((prev) => prev.map((img) => ({ ...img, isPrimary: img.id === id })));
  const handleDragOver   = (e: DragEvent) => { e.preventDefault(); setIsDragging(true); };
  const handleDragLeave  = () => setIsDragging(false);
  const handleDrop       = (e: DragEvent) => { e.preventDefault(); setIsDragging(false); handleFileSelect(e.dataTransfer.files); };
  const handleClickUpload = () => { if (canUpload && !hasReachedMax) fileInputRef.current?.click(); };
    ...
    <div className="space-y-4">
      {canUpload && (
        <div>
          {hasReachedMax ? (
            ...
            <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={handleClickUpload}
              ...
              <input ref={fileInputRef} type="file" accept={DEFAULT_UPLOAD_CONFIG.allowedTypes.join(',')} multiple onChange={(e) => handleFileSelect(e.target.files)} className="hidden" />
              <Upload className="w-10 h-10 mx-auto text-gray-400 mb-2" />
              <p className="text-sm font-medium text-gray-700 mb-1">Haz clic o arrastra imágenes aquí</p>
              <p className="text-xs text-gray-500">JPEG, PNG, WebP • Max 5MB • {allImages.length}/{maxImages} imágenes</p>
            </div>
          )}
        </div>
      )}
      ...
      {pendingCount > 0 && (
        <div className="flex items-center gap-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
          {uploadingCount > 0 ? <Loader2 className="w-4 h-4 animate-spin flex-shrink-0" /> : <AlertCircle className="w-4 h-4 flex-shrink-0" />}
          <span>{uploadingCount > 0 ? `Subiendo ${uploadingCount} imagen${uploadingCount > 1 ? 'es' : ''}…` : `${pendingCount} imagen${pendingCount > 1 ? 'es' : ''} pendiente${pendingCount > 1 ? 's' : ''} — se subirán al guardar`}</span>
        </div>
      )}
      ...
      {allImages.length > 0 && (
        <div>
          <p className="text-sm font-medium text-gray-700 mb-3">Imágenes del producto ({allImages.length}/{maxImages})</p>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {allImages.map((image) => {
              const isHovered   = hoveredImageId === image.id;
              const isUploading = uploadingIds.has(image.id);
              return (
                <div key={image.id} onMouseEnter={() => setHoveredImageId(image.id)} onMouseLeave={() => setHoveredImageId(null)}
                  ...
                  <div className="aspect-square bg-gray-100">
                    <img src={image.url} alt={image.alt || 'Producto'} className="w-full h-full object-cover"
                      onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/400x400/e5e7eb/64748b?text=Error'; }} />
                  </div>

                  {/* Overlay oscuro — solo NO principales */}
                  {!image.isPrimary && (
                    <div className="absolute inset-0 bg-black pointer-events-none transition-opacity" style={{ zIndex: 10, opacity: isHovered ? 0.5 : 0 }} />
                  )}

                  {/* Badge principal */}
                  {image.isPrimary && (
                    <div className="absolute top-2 left-2 flex items-center gap-1 bg-blue-600 text-white text-xs px-2 py-1 rounded shadow-lg" style={{ zIndex: 40 }}>
                  ...
                  {/* Badge pendiente */}
                  {isPending(image) && (
                    <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-amber-500 text-white text-xs px-2 py-1 rounded shadow-lg" style={{ zIndex: 40 }}>
                      {isUploading ? <><Loader2 className="w-3 h-3 animate-spin" /> Subiendo</> : <><CheckCircle className="w-3 h-3" /> Pendiente</>}
                    </div>
                  )}

                  {/* Botón eliminar */}
                  {canUpload && !isUploading && (
                    <button type="button" onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ isOpen: true, imageId: image.id, imageUrl: image.url }); }}
                      className="absolute top-2 right-2 p-1.5 bg-red-600 text-white rounded-md hover:bg-red-700 transition-all shadow-lg"
                      style={{ zIndex: 30, opacity: isHovered ? 1 : 0, visibility: isHovered ? 'visible' : 'hidden' }} title="Eliminar imagen">
                  ...
                  {canUpload && !image.isPrimary && !isUploading && (
                    <button type="button" onClick={(e) => { e.stopPropagation(); handleSetPrimary(image.id); }}
                      ...
                      style={{ zIndex: 20, opacity: isHovered ? 1 : 0, visibility: isHovered ? 'visible' : 'hidden' }}>
      ...
      {allImages.length === 0 && (
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
          ...
          <p className="text-sm text-gray-500">{canUpload ? 'No hay imágenes. Haz clic arriba para agregar.' : 'No hay imágenes.'}</p>
        </div>
      )}

      {deleteConfirm && (
        <ConfirmDialog isOpen={deleteConfirm.isOpen} title="¿Eliminar esta imagen?"
          message="La imagen se eliminará de la lista. Si ya estaba guardada, se borrará al guardar el producto."
          confirmLabel="Eliminar" cancelLabel="Cancelar" onConfirm={handleConfirmRemove} onCancel={() => setDeleteConfirm(null)} />
      )}
    </div>
  );
}
